      ...
      <div className="max-w-6xl mx-auto">
        ...
        {Object.entries(grouped).map(([statusKey, apps]) => (
          <div key={statusKey} className="mb-10">
            ...
            {apps.length === 0 ? (
              <div className="text-gray-500 text-sm italic">No {statusKey.toLowerCase()} applications yet.</div>
            ) : (
              <div className="grid gap-5 sm:grid-cols-1 md:grid-cols-2">
                {apps.map(app => (
                  <div key={app.id} className="bg-white border border-gray-200 rounded-xl p-5 shadow hover:shadow-md transition">
                    ...
                    <p className="text-xs text-gray-400 mt-4">Applied on: {new Date(app.applied_at).toLocaleDateString()}</p>
                  </div>
                ))}
              </div>
            )}
